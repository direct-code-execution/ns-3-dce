...
Direct Code Execution (DCE) is a module for `ns-3 <http://www.nsnam.org/>`_ 
that provides facilities to execute, 
existing implementations of userspace and kernelspace network protocols or applications without source code changes,
enabling reproducible network experimentation. 
For example, instead of using ns-3's implementation of a ping-like application 
(as the pseudo application provided by |ns3| **V4PingHelper**), 
you can use the real **ping** application. 
...
Currently the POSIX API (libc) is not fully supported by DCE. However there are 
already about 400 methods supported. As the goal of DCE is to allow to execute 
network applications, many methods related to the network are supported for 
example socket, connect, bind, listen, read, write, poll, select. 
