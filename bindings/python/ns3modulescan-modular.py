...
class PreScanHook:
    ...
    def __call__(self, module_parser,
        ...
        try:            
            print(pygccxml_definition.location.file_name)
...
def ns3_module_scan(top_builddir, module_name, headers_map, output_file_name, cflags):
    ...
    castxml_options = dict(
        include_paths=[top_builddir,top_builddir+"/../../../build/include/ns3-dev"],
         define_symbols={
    ...
    if not os.path.exists(scan_header):
        ...
        print("scan_header : " + scan_header)

    module_parser.parse_init([scan_header],
                             None, whitelist_paths=[top_builddir,top_builddir+"/../../../build/include/ns3-dev"],
