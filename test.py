    ...
    if os.path.exists(examples_to_run_path):
        ...
        for example_name, do_run, do_valgrind_run in cpp_examples:
            ...
            if os.path.exists(example_path):
                ...
                example_names_original.append((example_name_original, do_run, do_valgrind_run))
...
def make_paths():
    ...
    if sys.platform == "darwin":
        ...
        os.environ["DCE_PATH"] = os.pathsep.join([_f for _f in dce_paths if _f])

        dce_roots = [os.getenv('DCE_ROOT'), \
                     os.path.join(NS3_BUILDDIR), \
                     os.path.join(NS3_INSTALL_DIR)]
        os.environ['DCE_ROOT'] = os.pathsep.join([_f for _f in dce_roots if _f])
...
def run_tests():
    ...
    for directory in EXAMPLE_DIRECTORIES:
        ...
        example_directory   = os.path.join("example", directory)
        examples_to_run_path = os.path.join(example_directory, "examples-to-run.py")
        cpp_executable_dir   = os.path.join(NS3_BUILDDIR, 'bin')
    ...
    #
    if options.kinds:
        path_cmd = os.path.join("bin", test_runner_name + " --print-test-type-list")
        (rc, standard_out, standard_err, et) = run_job_synchronously(path_cmd, os.getcwd(), False, False)
        print(standard_out, standard_err)

    if options.list:
        if len(options.constrain):
            path_cmd = os.path.join("bin", test_runner_name + " --print-test-name-list --print-test-types --test-type=%s" % options.constrain)
        else:
            path_cmd = os.path.join("bin", test_runner_name + " --print-test-name-list --print-test-types")
        (rc, standard_out, standard_err, et) = run_job_synchronously(path_cmd, os.getcwd(), False, False)
    ...
    if len(options.suite):
        ...
        path_cmd = os.path.join("bin", test_runner_name + " --print-test-name-list")
        ...
        if len(options.constrain):
            path_cmd = os.path.join("bin", test_runner_name + " --print-test-name-list --test-type=%s" % options.constrain)
            (rc, suites, standard_err, et) = run_job_synchronously(path_cmd, os.getcwd(), False, False)
        else:
            path_cmd = os.path.join("bin", test_runner_name + " --print-test-name-list")
            (rc, suites, standard_err, et) = run_job_synchronously(path_cmd, os.getcwd(), False, False)
    ...
    if sys.platform != "win32":
        if 'SC_NPROCESSORS_ONLN'in os.sysconf_names:
            ...
            proc = subprocess.Popen("sysctl -n hw.ncpu", universal_newlines=True,
    ...
    for test in suite_list:
        ...
        if len(test):
            ...
            path_cmd = os.path.join("bin", test_runner_name + " --test-name=%s%s%s" % (test, multiple, fullness))
                ...
                for test, do_run, do_valgrind_run in example_tests:
                    ...
                    if test_name in ns3_runnable_programs:
                        if eval(do_run):
                            ...
                            job.set_is_pyexample(False)
                            job.set_display_name(os.path.basename(test))
                ...
                for test, do_run in python_tests:
                    ...
                    if test_name in ns3_runnable_scripts:
                        if eval(do_run):
                            ...
                            job.set_display_name(test_name)
                            job.set_tmp_file_name("")
                            job.set_cwd(testpy_output_dir)
                            job.set_basedir(os.getcwd())
                            job.set_tempdir(testpy_output_dir)
...
def main(argv):
    ...
    parser.add_option("-i", "--grind-xml", action="store_true", dest="valgrind_xml", default=False,
